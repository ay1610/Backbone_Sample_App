...
;breeze.AbstractDataServiceAdapter = (function () {
    ...
    ctor.prototype.executeQuery = function (mappingContext) {
        ...
        var params = {
            ...
            success: function (httpResponse) {
                var data = httpResponse.data;
                // binson
                if (httpResponse.data.d != undefined) {
                    data = httpResponse.data.d;
                }
                var resultSet;
                var x = "results";
                //collectionCallback({ results: data, inlineCount: data.__count });
                resultSet = searchresults(resultSet, httpResponse.data, x);
                if (resultSet != false && resultSet.length > 0)
                    data = resultSet;
                resultSet = null;
    ...
    //Helper functions Added by Binson     
    function searchresults(resultSet, obj, target) {
        var found = false;
        for (var k in obj) {
            if (obj.hasOwnProperty(k))
                if (k === target)
                    ...
                    var result = searchresults(resultSet + "." + k, obj[k], target);
                    if (result)
                        return result;
                }
        }
...
;(function (factory) {
    ...
    ctor.prototype.jsonResultsAdapter = new JsonResultsAdapter({
        ...
        visitNode: function (node, mappingContext, nodeContext) {
            if (node == null) return {};
            //var entityTypeName = MetadataStore.normalizeTypeName(node.$type); //binson
